...
race_2 - doing the same as for race_1, and replacing no applicable with not applicable

### question - there's a lot of what looks like ethnicity data in this variable
```{r}
demo_dat <- demo_dat %>% 
...
Ethnicity - replace all instances of different spellings or orders of hispanic/latino/a/x to Hispanic/Latin(a)(o)(x). Also, replace all instances of different spellings or orders of not hispanic/latino to Non-Hispanic/Latin(a)(o)(x) 

### question to come back to, how to have two conditions met in case_when? i.e. how to have that it both starts with no and contains hispa
```{r}
demo_dat <- demo_dat %>% 
  mutate(ethnicity_recode = case_when(str_starts(ethnicity,"hispa") ~ "hispanic/latin(a)(o)(x)", 
                                      str_starts(ethnicity, "no") ~ "non-hispanic/latin(a)(o)(x)",
...
```{r}
demo_dat %>% 
  count(ethnicity, ethnicity_recode) 
```
