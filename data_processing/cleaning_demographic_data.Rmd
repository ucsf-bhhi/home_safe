...
remove "'",", ca" and "ca" from the end of responses
```{r}
demo_dat <- demo_dat %>% 
  mutate(location_of_participation_recode = str_replace(location_of_participation, "'", "")) %>% 
  mutate(location_of_participation_recode = str_replace(location_of_participation, ", ca| ca|,ca", ""))
```

Check for removal of Ca
```{r}
demo_dat %>% 
  filter(str_detect(location_of_participation, ",")) %>% 
  count(location_of_participation, location_of_participation_recode)
```

remove special characters like ":" before laguna beach. Combine misspellings of bakersfield.Combine mispelling of barstow.
```{r}
demo_dat <- demo_dat %>% 
