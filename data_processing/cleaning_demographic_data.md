...
- “cal city”, “california city” - do we think these are the same? (Made
  the change in the recode for now)
- responses i.e. “coulterville/la grange” (so combo of two cities) -
  leave as is?
- “north side of parking lot” - ?
- “sin city” - might be sun city? (there is no sin city in california it
  seems)
- “ylp” - ?
- home, homeless - are in the data still
- outside california - ? exclude?
...
“unknown”. Combining domestic partnership with married. Changing
gay/lesbian and straight/heterosexual to unknown.

### Question:

- How would we like to recode domestic partnership? (Lump with married?
  did this for now)
- What to do with single? (100/103 records with single are from LA)

``` r
demo_dat <- demo_dat %>% 
  mutate(current_marital_recode = case_when(str_detect(current_marital_status, "straight|gay|unknown|#|applicable|data not|1|2|3|4|5|6|7|8|9|0") ~ "unknown",
...
### Question - trying to plot the various income (income from benefits, income from pay, other income), but I think these are chr variables?

``` r
#demo_dat %>% 
#  ggplot() %>% 
#  geom_histogram(income_from_benefits)
```
