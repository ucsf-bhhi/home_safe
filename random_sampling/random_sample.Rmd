...
op_2_inver_dates <- op_2_inver_dates %>% 
  filter(date >= lubridate::ymd("2022-10-01"))

summary(op_2_inver_dates$date)
```

For some strange reason, there is a date in year `3030`--I will drop this and then use applicable observations for the filter.

```{r}
op_2_inver_dates <- op_2_inver_dates %>% 
  filter(date < lubridate::ymd("2024-01-01")) # Check in w/ Angelica
...
In order to limit the sampling to the most **recent** case per ID, I will first, `group_by` `id`, then sort descending by `case_start_sample_subsete_recode`, and finally `slice` for the first occurrence (i.e. most recent case by `id`).
...
sample_subset %>% 
    filter(id_count > 1) %>% 
    group_by(id) %>% 
    arrange(id, desc(case_start_sample_subsete_recode)) %>% 
    head()

sample_subset_subset <- sample_subset %>% 
  group_by(id) %>% 
  arrange(id, desc(case_start_sample_subsete_recode)) %>% 
  slice(1) %>% 
  ungroup()
...
```{r, message = FALSE}
sample_subset_subset <- sample_subset_subset %>% 
  add_count(reporting_agency) %>% 
  mutate(`agency_size` = if_else(n <= 10, "small_agency", "not_small"))

small_agencies <- sample_subset_subset %>% 
  filter(agency_size == "small_agency") %>% 
  droplevels()

all_other_agencies <- sample_subset_subset %>% 
...
#writexl::write_xlsx(output, paste(path_to_proj, "/Quantitative/sample_subseta/random_sample_iteration_1/stats_randomized_sample.xlsx", sep = ""))

#write.csv(raw_sample, paste(path_to_proj, "/Quantitative/sample_subseta/random_sample_iteration_1/randomized_sample.csv", sep = ""), row.names = F)

#write.csv(raw, paste(path_to_proj, "/Quantitative/sample_subseta/random_sample_iteration_1/og_file_used.csv", sep = ""))
```
